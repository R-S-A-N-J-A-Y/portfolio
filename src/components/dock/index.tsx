...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden md:flex h-16 gap-4 items-end rounded-2xl bg-neutral-900 px-4 pb-3 max-w-screen-md",
        className,
      )}
    ...
    <div>
      <Link href={href}>
        <motion.div
          ...
          className="flex items-center justify-center w-12 h-12 bg-neutral-800 rounded-full relative"
        >
          <AnimatePresence>
            {hovered && (
              <motion.div
                ...
                className="absolute left-1/2 -translate-x-1/2 -top-8 w-fit text-xs text-white px-2 py-1 bg-neutral-700 border border-neutral-900 rounded-md"
              >
                {title}
              </motion.div>
            )}
